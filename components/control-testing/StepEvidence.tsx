...
/** Step 4: the evidence list for this test - same shape as the change lab's evidence step (components/change-lab/StepSupportingData.tsx). */
export default function StepEvidence({ evidence, loading, readOnly, onAdd }: StepEvidenceProps) {
  const [showAdd, setShowAdd] = useState(false);
  const [draft, setDraft] = useState({ type: "test_result", title: "", description: "", linkUrl: "", evidenceDate: "" });
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (ok) {
        setDraft({ type: "test_result", title: "", description: "", linkUrl: "", evidenceDate: "" });
        setShowAdd(false);
      } else {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-text-muted">
          Attach the working papers, sample data, screenshots or sign-offs that support this test.
        </p>
      </div>
