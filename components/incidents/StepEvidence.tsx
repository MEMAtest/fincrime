...
/** Step 6: the evidence list for this incident - same shape as the control-testing evidence step (components/control-testing/StepEvidence.tsx). */
export default function StepEvidence({ evidence, loading, readOnly, onAdd }: StepEvidenceProps) {
  const [showAdd, setShowAdd] = useState(false);
  const [draft, setDraft] = useState({ type: "incident_report", title: "", description: "", linkUrl: "", evidenceDate: "" });
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (ok) {
        setDraft({ type: "incident_report", title: "", description: "", linkUrl: "", evidenceDate: "" });
        setShowAdd(false);
      } else {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-text-muted">
          Attach the working papers, correspondence, screenshots or sign-offs that support this incident record.
        </p>
      </div>
