...
import { Target, Database, Cpu, GitBranch, BarChart3, ClipboardCheck, BookOpen, Scale } from "lucide-react";
...
function Chips({ label, values }: { label: string; values: string[] }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {values.map((v) => (
        <span key={v} className="inline-flex items-center px-2 py-0.5 rounded-full border border-white/10 bg-white/[0.03] text-[11px] text-foreground">{v}</span>
      ))}
    </div>
  );
}
...
/**
 * The shared typology detail content (header block + KeyTerms + the 4 tabs).
 * Rendered on the SEO page (`/typology-iq/t/[slug]`) AND inside `TypologyDetailModal`.
 * Page-only chrome (back link, related typologies, next steps) stays in the page.
 * `compact` drops the big header for use inside a modal that already has one.
 */
export default function TypologyDetail({ typology, compact = false }: { typology: Typology; compact?: boolean }) {
    ...
    <div>
      {!compact && (
        <div className="glass-card rounded-2xl p-6 sm:p-8 mb-6">
          <div className="flex items-start gap-4">
            <div className="shrink-0"><RiskThemeIcon riskTheme={typology.riskTheme} size="md" animated={false} /></div>
            <div className="min-w-0">
              <span className="inline-block text-[10px] uppercase tracking-wider font-medium mb-1" style={{ color: cfg.primary }}>{cfg.label}</span>
      ...
      {compact && (
        <div className="mb-4 space-y-2">
          <p className="text-sm text-text-muted leading-relaxed">{typology.description}</p>
          <Chips label="Firm types" values={typology.applicableFirmTypes.map((f) => FIRM_TYPE_LABEL[f] ?? f)} />
        </div>
      )}

      {!compact && <KeyTerms terms={["typology", "transaction monitoring", "SAR", "EDD", "red-flag indicator"]} />}
        ...
        tabs={[
          {
            ...
            content: (
              <ResultsGrid>
                <ResultCard title="What it is" icon={BookOpen} className="md:col-span-2" index={0}><p className="leading-relaxed">{typology.description}</p></ResultCard>
                <ResultCard title="Control objective" icon={Target} className="md:col-span-2" index={1}><p className="leading-relaxed">{typology.controlObjective}</p></ResultCard>
                <ResultCard title="Data required" icon={Database} className="md:col-span-2" index={2}>
                  <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-1.5">
                    {typology.dataRequired.map((d) => (<li key={d} className="flex items-start gap-2"><span className="w-1.5 h-1.5 rounded-full bg-accent mt-2 shrink-0" /><span>{d}</span></li>))}
                  </ul>
                </ResultCard>
              </ResultsGrid>
            ),
          ...
          {
            id: "detection", label: "Red flags & detection", icon: Cpu,
            content: (
              <ResultsGrid>
                  ...
                  <div className="space-y-3">
                    {typology.detectionLogic.map((r) => (
                      <div key={r.id} className="p-3 bg-slate-50 rounded-lg">
                        <div className="flex items-center justify-between mb-1"><span className="font-mono text-xs text-slate-400">{r.id}</span><Badge variant={priorityVariant(r.priority)}>{r.priority}</Badge></div>
          ...
          {
            id: "workflow", label: "Workflow & governance", icon: GitBranch,
            content: (
              <ResultsGrid>
                  ...
                  <div className="space-y-3">
                    {typology.workflowSteps.map((w) => (
                      <div key={w.step} className="flex gap-3">
                        ...
                        <div><p className="font-medium text-slate-800 text-sm">{w.title}</p><p className="text-xs text-slate-500 mt-0.5">{w.description}</p>
                          <div className="flex gap-3 mt-1">{w.sla ? <span className="text-xs text-accent">SLA: {w.sla}</span> : null}<span className="text-xs text-slate-400">{w.responsible}</span></div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-3">
                    {typology.metrics.map((m) => (
                      <div key={m.name} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                        <div><p className="font-medium text-slate-800 text-sm">{m.name}</p><p className="text-xs text-slate-400">{m.description}</p></div>
                  ...
                  <div className="space-y-2">
                    {typology.governanceChecklist.map((g) => (
                      <div key={g.id} className="flex items-start gap-2 py-2 border-b border-slate-100 last:border-0">
                        ...
                        <div><p className="text-sm text-slate-700">{g.item}</p><div className="flex gap-2 mt-0.5"><span className="text-xs text-accent">{g.frequency}</span><span className="text-xs text-slate-400">Owner: {g.owner}</span></div></div>
                      </div>
                    ))}
                  </div>
                </ResultCard>
          ...
          {
            id: "evidence", label: "Evidence & sources", icon: Scale,
            content: (
