...
import { computeGaps } from "./journeyCompute";
import type {
  ActionPriority,
...
interface StepDecisionProps {
  ...
  onSaveRecommendation: (text: string) => Promise<unknown>;
  onSubmitForReview: () => Promise<unknown>;
  onCreatePerson: (input: { name: string; role: PersonRole; email?: string }) => Promise<PersonDTO | null>;
  onRecordDecision: (payload: DecisionPayload) => Promise<boolean>;
}
...
/** Step 7: recommendation, an approver-signed decision (approve / approve with conditions / reject), conditions and follow-up actions. */
export default function StepDecision({
  ...
  const addCondition = (description = "") =>
    setConditions((prev) => [...prev, { key: makeTempKey(), description, dueDate: "", ownerPersonId: "" }]);
  const addAction = (title = "") =>
    setActions((prev) => [...prev, { key: makeTempKey(), title, ownerPersonId: "", dueDate: "", priority: "medium" }]);

  const prefillConditionsFromGaps = () => {
    gaps.forEach((gap) =>
      addCondition(
        gap.kind === "no_controls"
          ? `Attach a control to mitigate "${gap.riskTitle}"`
          : `Remediate gap coverage on "${gap.controlLabel}" for "${gap.riskTitle}"`
      )
    );
  };
  const prefillActionsFromGaps = () => {
    gaps.forEach((gap) =>
      addAction(
        gap.kind === "no_controls"
          ? `Design and attach a control for "${gap.riskTitle}"`
          : `Close the gap on "${gap.controlLabel}" (${gap.riskTitle})`
      )
    );
  };
      ...
      const payload: DecisionPayload = {
        ...
        conditions: conditions
          ...
          .map((c) => ({
            ...
            ownerPersonId: c.ownerPersonId || null,
          })),
        actions: actions
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-foreground mb-1">Recommendation &amp; decision</h2>
        <p className="text-sm text-text-muted">
          Record a recommendation, submit the assessment for review, and capture the approver&apos;s decision with
          any conditions and follow-up actions.
        </p>
      </div>

      <div>
        <label className="text-[11px] uppercase tracking-wider text-text-muted">Recommendation</label>
        <textarea
          key={assessment.id}
          defaultValue={assessment.recommendation ?? ""}
          onBlur={(e) => {
            if (e.target.value !== (assessment.recommendation ?? "")) void onSaveRecommendation(e.target.value);
          }}
          rows={4}
          placeholder="What are you recommending, and why?"
          className="w-full mt-1.5 px-3.5 py-2.5 rounded-lg border border-line-2 bg-surface text-foreground text-sm placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-colors"
        />
      </div>

      {assessment.status === "draft" && (
        <div className="glass-card rounded-xl p-4 flex items-center justify-between gap-3">
          <p className="text-sm text-text-muted">Ready for an approver to review this assessment?</p>
          <Button
            size="sm"
      ...
      {assessment.status === "in_review" && !decision && (
        <div className="glass-card rounded-xl p-4">
          <Badge variant="info">Awaiting decision</Badge>
          <p className="text-sm text-text-muted mt-2">
            This assessment is in review. Record the decision below once the approver has reached one.
          </p>
        </div>
      )}
          ...
          {outcome === "approve_with_conditions" && (
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-3">
                  {gaps.length > 0 && (
                    <button
                      type="button"
                      onClick={prefillConditionsFromGaps}
                      className="inline-flex items-center gap-1 text-xs font-medium text-accent hover:underline cursor-pointer"
                    >
                      <Sparkles className="h-3 w-3" /> Prefill from {gaps.length} gap{gaps.length === 1 ? "" : "s"}
                    </button>
                  )}
                  <button
                    type="button"
              ...
              <div className="space-y-2">
                {conditions.map((c) => (
                  <div key={c.key} className="flex flex-wrap items-center gap-2 p-2.5 rounded-lg bg-white/[0.02] border border-white/10">
                    <input
                      value={c.description}
                      onChange={(e) =>
                        setConditions((prev) => prev.map((x) => (x.key === c.key ? { ...x, description: e.target.value } : x)))
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setConditions((prev) => prev.map((x) => (x.key === c.key ? { ...x, ownerPersonId: e.target.value } : x)))
                      }
                      className="px-2.5 py-1.5 rounded-lg border border-line-2 bg-surface text-foreground text-xs focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
                    >
                ...
                {conditions.length === 0 && (
                  <p className="text-xs text-text-muted">At least one condition is required for this outcome.</p>
                )}
              </div>
            </div>
          )}
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-3">
                {gaps.length > 0 && (
                  <button
                    type="button"
                    onClick={prefillActionsFromGaps}
                    className="inline-flex items-center gap-1 text-xs font-medium text-accent hover:underline cursor-pointer"
                  >
                    <Sparkles className="h-3 w-3" /> Prefill from {gaps.length} gap{gaps.length === 1 ? "" : "s"}
                  </button>
