...
export function generateControlRegisterPDF(data: ControlRegisterPDFData): Buffer {
  ...
  entries.forEach(({ control: c, override: o }, idx) => {
    ...
    const rows: [string, string][] = [
      ...
      ...(c.plainObjective ? ([["What it's for", c.plainObjective]] as [string, string][]) : []),
      ...(c.plainHowItWorks ? ([["How it works", c.plainHowItWorks]] as [string, string][]) : []),
      ["Control objective", c.objective],
      ["Rule logic", c.ruleLogic],
      ["Threshold", eff(o?.threshold, c.defaultThreshold)],
      ...(c.plainWhyThreshold ? ([["Why this threshold", c.plainWhyThreshold]] as [string, string][]) : []),
